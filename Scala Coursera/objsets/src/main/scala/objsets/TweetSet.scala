...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = {
    if(!left.isEmpty && !right.isEmpty) {
      if (right.mostRetweeted.retweets > left.mostRetweeted.retweets && right.mostRetweeted.retweets > elem.retweets)
        right.mostRetweeted
      else if (left.mostRetweeted.retweets > elem.retweets)
        left.mostRetweeted
      else
        elem
    }

    else if(!left.isEmpty && left.mostRetweeted.retweets > elem.retweets)
        left.mostRetweeted
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(t => google.exists(word => t.text contains(word)) == true)
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(t => apple.exists(word => t.text contains(word)) == true)
