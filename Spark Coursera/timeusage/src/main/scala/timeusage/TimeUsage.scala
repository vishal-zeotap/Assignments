...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def go(ll: List[String]): List[Column] = columnNames.filter(s => ll.map(x => s.startsWith(x)).reduce(_ || _)).map(new Column(_))

    (go(primary), go(work), go(leisure))
  }
  ...
  def timeUsageSummary(
    ...
    val otherProjection: Column = otherColumns.map(_ / 60).reduce(_ + _).alias("others")
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    val df = summed.groupBy($"working",$"age",$"sex").avg()
      .orderBy("age").orderBy("sex").orderBy("working").cache()

      df.select(
        col("working"), col("sex"), col("age"),
        round(col("avg(primaryNeeds)"),1).as("primaryNeeds"),
        round(col("avg(work)"),1).as("work"),
        round(col("avg(others)"),1).as("other")
      )
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(x=>(x.working,x.sex,x.age))
      .agg(typed.avg[TimeUsageRow](x=>round(x.primaryNeeds)),typed.avg[TimeUsageRow](x=>round(x.work)),typed.avg[TimeUsageRow](x=>round(x.other)))
      .map(x=>new TimeUsageRow(x._1._1,x._1._2,x._1._3,x._2,x._3,x._4))
