...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val q = postings.filter(_.postingType == 1).map(posting => (posting.id, posting))
    val a = postings.filter(_.postingType == 2).map(posting => (posting.parentId.get, posting))
    q.join(a).groupByKey()
  }
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map(elem => (firstLangInTag(elem._1.tags, langs).getOrElse(-1) * langSpread, elem._2)).filter(_._1 >= 0).cache()
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val clustered = vectors.map(point => (findClosest(point, means), point))
